...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    // Helper to collect word refs after render
    const WordsCollector = ({ onReady, sectionIndex }: { onReady: (idx: number) => void, sectionIndex: number }) => {
      useEffect(() => {
    ...
    const measureAndCenterLists = (toIndex = index, animate = true) => {
      const centerTrack = (
        ...
        const track = isRight ? rightTrackRef.current : leftTrackRef.current;
        if (!track) return;
    ...
    return (
      ...
      >
        ...
        <div className="fx-scroll">
          <div className="fx-fixed-section" ref={fixedSectionRef} style={{ height: `${(total + 1) * 100}vh` }}>
                          ...
                          <WordsCollector
                            ...
                            onReady={(idx) => {
                              if (tempWordBucket.current.length) {
                                wordRefs.current[idx] = [...tempWordBucket.current];
                              }
                              tempWordBucket.current = [];
                            }}
                          />
          ...
          <div className="fx-end">
            <p className="fx-fin">PROTOCOL SECURED</p>
          </div>
        </div>
      </div>
    );
